...
const FlagCard = ({ flag, showMunicipality = false, index = 0 }) => {
  ...
  // A flag is revealed if:
  // 1. User owns the first NFT, OR
  // 2. User owns the second NFT, OR
  // 3. The flag pair is complete (anyone can see it)
  const isRevealed = (() => {
    // If pair is complete, everyone can see it
    if (flag.is_pair_complete) return true;

    // If user has shown interest, they can see the card (to encourage claiming)
